...
case $1 in
  major)
    ...
    ;;
  minor)
    perl -i -pe 's/^(version:\s+)(\d+)\.(\d+)\.(\d+)\+(\d+)$/"${1}".$2.".".($3+1).".0+1"/e' ./pubspec.yaml
    ;;
  patch)
    perl -i -pe 's/^(version:\s+)(\d+)\.(\d+)\.(\d+)\+(\d+)$/"${1}".$2.".".$3.".".($4+1)."+1"/e' ./pubspec.yaml
    ;;
  build)
...
git commit -m "Bump version to $versionWithPrefix" ./pubspec.yaml
git tag $versionWithPrefix
